...
import com.example.mystory.data.database.RemoteKeys
...
class UserRemote (
    ...
    override suspend fun load(
        ...
        return try {
            val responseData = apiService.getStoriesForPaging(token, page, state.config.pageSize)
            val endOfPaginationReached = responseData.listStory.isEmpty()

            val storyDao = database.storyDao()
            val remoteKeysDao = database.remoteKeyDao()
            ...
            database.withTransaction {
                ...
                Log.d("Ping!", responseData.listStory.toString())
                storyDao.insertStory(responseData.listStory)
    ...
    private suspend fun getRemoteKeyClosestToCurrentPosition(state: PagingState<Int, ListItem>): RemoteKeys? {
